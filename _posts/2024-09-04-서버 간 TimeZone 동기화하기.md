...
이번 문제상황을 겪게된 환경에서 사용했던 MySQL에서는 시간 정보를 어떻게 저장하는지 한번 살펴보겠습니다.
...
DATETIME은 날짜와 시간만을 저장하여 TimeZone에 따라 다르게 해석될 수 있는 형식이고   
...
여기까지 본다면 "TIMESTAMP를 사용하여 시점을 저장할 때는 TimeZone을 고려해야 하고 DATETIME으로 시간을 저장할 때는 TimeZone을 고려하지 않아도 되겠구나!" 
라고 생각이 드는데요.   
...
1. 알림을 보낼 때 서버에서 알림 센터 메시지 생성(어플리케이션에선 LocalDateTime으로 생성 시간 저장, DB에는 DATETIME 타입으로 저장)
2. DB에 메시지 저장
3. 유저의 알림 센터 메시지 조회
...
## 원인 분석
이는 DB에서 다르게 결과를 읽어오는 것이 아니라 **JDBC의 세션 TimeZone과 JVM의 TimeZone에 따라서 데이터를 변환시키기 때문입니다.**

MySQL에서 DATE나 DATETIME은 분명 TimeZone 정보와 무관한 데이터가 맞지만 
JDBC와 JVM의 각 TimeZone 설정에 따라 저장된 데이터와 다르게 읽힐 수 있다는 것입니다.

그러니까 MySQL의 공식문서의 일부만 보고   
"DATE와 DATETIME 형식의 데이터를 읽어올 때 TimeZone은 전혀 신경쓰지 않아도 되겠구나!" 라고 생각하면 혼란이 올 수 있습니다.(제가 그런 혼란이 왔습니다..)

ConnectorJ에 관련된 MySQL 문서를 보면 아래와 같이 설명합니다.

> the result is that, when retrieved **in a different session with a different time zone offset** as the specified one, the DATETIME value becomes **different from the original** input value.

그러니까 앞에서 DATETIME이 TimeZone과 무관하다는 이야기는 JVM, JDBC의 TimeZone이 동일할 떄를 전제로 하는 것입니다.

이를 이해하고 이 문제상황을 다시 정리해보겠습니다.
...
하지만 아무리 생각해도 JVM과 JDBC 세션 간 TimeZone도 동기화를 시켜놨고   
조회도 아니고 저장을 하는데 있어서 변환이 될만한 이유는 도통 떠오르지 않았습니다..

그래서 이래저래 구글링을 해본 결과 stackoverflow에서 답을 찾을 수 있었습니다..!

## 원인 분석 및 해결

[stackoverflow 답변](https://stackoverflow.com/questions/54418827/mysql-date-changes-to-yesterdays-date-after-jpa-save)

바로 DB Connection을 관리하는 **hikariCP에서 JVM의 TimeZone 정보를 캐싱**을 하는 기능이 있는데 default 값이 true이기 떄문에   
...
Hibernate 코드를 정확하게 봐야 알 수 있겠지만   
...
<figure>
    ...
    <figcaption>찾아보니 실제로 unwrap 메서드에서 Date타입의 변환을 위해 valueOf를 사용합니다!</figcaption>
</figure>
...
먼저, LocalDate 정보가 Date로 바뀌면서 Time과 TimeZone이 추가된 정보로 바뀝니다.(valueOf의 영향)   

**_2024-09-03_** ➡️ **_2024-09-03 00:00:00(UTC+9)_**

하지만 JDBC에서는 cacheDefaultTimezone=true가 디폴트이기 때문에 캐싱한 TimeZone 정보를 UTC 기준으로 해석하여 이시간이 하루 이르게 변합니다.   
**_2024-09-03 00:00:00(UTC+9)_** ➡️ **_2024-09-02 15:00:00(UTC+0)_**

따라서 세션 시간대와는 관련 없이 캐싱 정보의 불일치 문제로 하루 이르게 저장이 되는 것 입니다.
...
사실 해결방법은 간단하지만 그 원인을 알기 위해서 어플리케이션과 DBMS의 시간 데이터 처리 방법에 대해 많이 알아볼 수 있는 경험이었습니다.🙂
...
4. **LocalDateTime**(날짜, 시간정보)을 저장할 때는 **세션 TimeZone**을 기준으로 하고 **LocalDate**(날짜정보)를 저장할 땐 **JVM TimeZone**을 기준으로 한다. (날짜만 저장할 시에는 변경한 Time정보를 복구할 수 없기 때문, 8.0.20 기준이다.)
5. DB에서 DATETIME/DATE 타입의 데이터를 조회할 때 세션과 JVM TimeZone이 다르다면 **JVM 기준으로 정보를 변환**한다.
6. HikariCP에서 JVM TimeZone 정보를 캐싱하는 것이 주의하자(캐싱이 디폴트, 정보 불일치 주의, 특히 JVM을 기준으로 저장하는 LocalDate의 경우)

👏 JVM이나 세션 TimeZone은 UTC로 통일하고 필요 시에 후처리를 하자‼️
...
[mysql 공식문서 - Preserving Time Instants](https://dev.mysql.com/doc/connector-j/en/connector-j-time-instants.html)   
